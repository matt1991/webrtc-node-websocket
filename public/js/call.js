...
var Call = function(params) {
  ...
  this.channel_.onacceptmessage = this.onAcceptMessage_.bind(this);
  this.channel_.oninvitemessage = this.onInviteMessage_.bind(this);
...
Call.prototype.onAcceptMessage_ = function() {
...
Call.prototype.onInviteMessage_ = function() {
...
Call.prototype.register = function(userName) {

  this.params_.clientId = userName;
  ...
  Promise.all([channelPromise]).then(function() {
    this.channel_.register(this.params_.clientId);
  }.bind(this)).catch(function(error) {
    this.onError_('WebSocket register error: ' + error.message);
  }.bind(this));
};

Call.prototype.inviteRemoteUser = function(remoteUserName) {
  this.params_.toId = remoteUserName;
  this.params_.isInitiator = true;
  // Asynchronously open a WebSocket connection to WSS.
  // TODO(jiayl): We don't need to wait for the signaling channel to open before
  // start signaling.
  this.channel_.inviteRemoteUser(remoteUserName);
};

Call.prototype.acceptInvitation = function() {
  this.channel_.acceptInvitation();
  this.params_.isInitiator = false;
